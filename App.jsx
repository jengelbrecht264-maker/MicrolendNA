...
const BorrowerCreditScore = ({ user, borrower, setView }) => {
        ...
        <div style={{ display: "flex", alignItems: "flex-start", gap: 24, position: "relative", zIndex: 1 }}>
          ...
          <div style={{ flexShrink: 0 }}>
            {notified ? (
              ...
              <button onClick={() => setNotified(true)} style={{ padding: "12px 24px", background: DS.colors.gold, color: "#0A0F1E", borderRadius: 10, border: "none", fontWeight: 700, fontSize: 14, cursor: "pointer" }}>
                🔔 Notify Me on Launch
              </button>
            )}
          </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, marginBottom: 28 }}>
        {pricingTiers.map((tier, i) => (
          <div key={i} style={{ padding: 28, background: DS.colors.surface, border: `2px solid ${tier.color}44`, borderRadius: 20, position: "relative", boxShadow: i === 1 ? `0 0 40px ${DS.colors.gold}15` : "none" }}>
            ...
            <button onClick={() => setNotified(true)} style={{ width: "100%", padding: 12, borderRadius: 10, border: `1px solid ${tier.color}`, background: i === 1 ? tier.color : "transparent", color: i === 1 ? "#0A0F1E" : tier.color, fontWeight: 700, fontSize: 14, cursor: "pointer" }}>
  ...
  return (
    <div className="fade-in">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, marginBottom: 20 }}>
        ...
        <Card>
          <h3 style={{ fontFamily: "'Syne',sans-serif", fontWeight: 700, marginBottom: 16, fontSize: 15 }}>Financial Information</h3>
          {editMode ? (
            <>
...
const BorrowerDocs = ({ borrower, setBorrower, showToast }) => {
  ...
  const handleFileChange = (key, event) => {
    ...
    setUploading(key);
    // Read file as data URL (simulates secure upload)
    ...
    reader.onload = async function(e) {
      ...
      setUploadedFiles(function(prev) {
        ...
        n[key] = Object.assign({ dataUrl: dataUrl }, meta);
        return n;
      });
      ...
      // 2. Persist document to storage (source of truth)
      try {
        await StorageService.saveDocument(uid, key, meta, dataUrl);
        // 3. Persist updated borrower profile (with new doc in list)
        await StorageService.saveBorrowerProfile(uid, updated);
        // 4. Sync to LENDER_DB so lender/admin can see doc count
        StorageService.syncToLenderDB(uid, updated);
      } catch (storageErr) {
        // Document is in session memory; warn user that it won't survive refresh
        showToast("File stored in session only — storage limit may have been reached", "warning");
      }
    };
    reader.onerror = function() { setUploading(null); showToast("Error reading file", "error"); };
...
const BorrowerApply = ({ borrower, user, showToast, setView }) => {
  ...
  const handleSubmit = () => {
    ...
    var activeLenders = DB.lenders.filter(function(l) { return l.status === "active"; });
    var matchedLender = null;
    for (var i = 0; i < activeLenders.length; i++) {
      var l = activeLenders[i];
      var prefs = LENDER_DB.lenderPrefs[l.id];
      if (!prefs) { matchedLender = l; break; }
      if (prefs.acceptedTiers && prefs.acceptedTiers.indexOf(tier) < 0) continue;
      if (prefs.minSalary && borrower.salary < prefs.minSalary) continue;
      if (prefs.minLoanAmount && amt < prefs.minLoanAmount) continue;
      if (prefs.maxLoanAmount && amt > prefs.maxLoanAmount) continue;
      if (prefs.firstBorrowerAllowed === false && borrower.firstBorrower) continue;
      if (prefs.requireKYC && borrower.kycStatus !== "verified") continue;
      matchedLender = l; break;
    }

    // PAYG charge on routing
    var charge = (matchedLender && matchedLender.plan === "payasyougo") ? DB.riskRules.payAsYouGoFee : 0;

    // Create lead in lender queue
    var newApp = {
      id: "ap" + Date.now(),
      borrowerId: borrower.id,
      borrowerName: borrower.name,
      tier: tier,
      riskScore: borrower.riskScore || 65,
      amount: amt,
      term: +form.term,
      ...
      docs: (borrower.documents || []).length,
      kycStatus: borrower.kycStatus || "pending",
      amlStatus: borrower.amlStatus || "clear",
      bankVerified: borrower.bankVerified || false,
      firstBorrower: borrower.firstBorrower,
      lenderId: matchedLender ? matchedLender.id : null,
      channel: "platform",
    };
    LENDER_DB.applications.push(newApp);

    // Also push to DB.applications so the borrower can track it under "My Applications"
    var appRecord = {
      id: newApp.id,
      borrowerId: borrower.id,
      ...
      riskScore: borrower.riskScore || 65,
      matchedLender: matchedLender ? matchedLender.name : null,
    };
    DB.applications.push(appRecord);
    // Persist to storage so application survives refresh/re-login
    StorageService.saveApplication(appRecord).catch(function() {});
    // Tag LENDER_DB app with borrowerUserId so lender portal can link to storage profile
    newApp.borrowerUserId = user.id;

    // Update lender stats
    if (matchedLender) {
      var rec = DB.lenders.find(function(l) { return l.id === matchedLender.id; });
      if (rec) {
        rec.leadsTotal = (rec.leadsTotal || 0) + 1;
        rec.leadsPending = (rec.leadsPending || 0) + 1;
        if (matchedLender.plan === "payasyougo") {
          rec.revenue = (rec.revenue || 0) + charge;
        }
      }
      DB.notifications.push({
        id: "n" + Date.now(),
        userId: matchedLender.id,
        msg: "New " + tier + "-tier lead: " + borrower.name + " — N$" + amt.toLocaleString() + " " + form.purpose,
        read: false,
        time: "just now",
      });
    }

    // Determine lender's rate for this tier
    var lenderRate = DB.riskRules.interestRate[tier] || 30;
    if (matchedLender && LENDER_DB.lenderPrefs[matchedLender.id]) {
      var lprefs = LENDER_DB.lenderPrefs[matchedLender.id];
      if (lprefs.interestRates && lprefs.interestRates[tier]) {
        lenderRate = lprefs.interestRates[tier];
      }
    }
    ...
    setStep(3);
    var msg = matchedLender
      ? "Application submitted — matched to " + matchedLender.name + "!"
      : "Application submitted — pending lender assignment.";
    showToast(msg)
  };
          ...
          {borrower ? (
            <>
              ...
              {(() => {
                ...
                const _rate = DB.riskRules.interestRate[_t] || 30;
                return (
                  <div style={{ padding: 16, background: DS.colors.accentDim, borderRadius: 8, marginBottom: 20 }}>
                    <div style={{ display: "flex", justifyContent: "space-between" }}>
                      <div><p style={{ fontSize: 12, color: DS.colors.textMuted }}>Your Tier</p>{_t !== "—" ? <TierBadge tier={_t} /> : <span style={{ color: DS.colors.textMuted, fontSize: 13 }}>Save profile first</span>}</div>
                      ...
                      <div><p style={{ fontSize: 12, color: DS.colors.textMuted }}>Interest</p><p style={{ fontWeight: 700, fontSize: 20 }}>{_rate}% p.a.</p></div>
                    </div>
                  </div>
                );
              })()}
        ...
        <Card className="fade-in">
          ...
          {form.amount && form.term && (
            <div style={{ padding: 14, background: DS.colors.surfaceAlt, borderRadius: 8, marginBottom: 16 }}>
              <p style={{ fontSize: 12, color: DS.colors.textMuted, marginBottom: 8 }}>REPAYMENT SUMMARY</p>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
                <div><p style={{ fontSize: 11, color: DS.colors.textMuted }}>Principal</p><p style={{ fontWeight: 700 }}>N${(+form.amount).toLocaleString()}</p></div>
                <div><p style={{ fontSize: 11, color: DS.colors.textMuted }}>Monthly Payment</p><p style={{ fontWeight: 700, color: DS.colors.accent }}>N${(+form.amount * (1 + (DB.riskRules.interestRate[borrower?.tier || "C"] || 30) / 100) / +form.term).toFixed(2)}</p></div>
                <div><p style={{ fontSize: 11, color: DS.colors.textMuted }}>Total Cost</p><p style={{ fontWeight: 700 }}>N${(+form.amount * (1 + (DB.riskRules.interestRate[borrower?.tier || "C"] || 30) / 100)).toFixed(2)}</p></div>
              </div>
            </div>
          )}
      ...
      {step === 3 && result && (
        <Card className="fade-in" style={{ textAlign: "center" }}>
          ...
          <p style={{ color: DS.colors.textSecondary, marginBottom: 24, fontSize: 14 }}>
            {result.matchedLender
              ? "Matched to " + result.matchedLender
              : "Application queued — a lender will be assigned shortly."}
          </p>

          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 10, marginBottom: 20 }}>
            {[
              { l: "Credit Tier", v: "Tier " + result.tier, c: DS.colors["tier" + result.tier] || DS.colors.accent },
              { l: "Matched Lender", v: result.matchedLender || "Pending", c: DS.colors.info },
              { l: "Amount", v: "N$" + result.amount.toLocaleString(), c: DS.colors.accent },
              { l: "Term", v: result.term + " months", c: DS.colors.textPrimary },
              { l: "Interest Rate", v: result.rate + "% p.a.", c: DS.colors.gold },
              { l: result.charge > 0 ? "PAYG Charge" : "Billing", v: result.charge > 0 ? "N$" + result.charge : "Subscription", c: result.charge > 0 ? DS.colors.warning : DS.colors.accent },
          ...
          <div style={{ padding: "14px 18px", background: DS.colors.accentDim, border: "1px solid " + DS.colors.accent + "33", borderRadius: 12, marginBottom: 16, textAlign: "left" }}>
            ...
            <p style={{ fontSize: 13, color: DS.colors.textSecondary, lineHeight: 1.7 }}>
              1. {result.matchedLender || "A lender"} receives your full profile and risk report.<br />
              2. Lender reviews within 24 hours and contacts you directly.<br />
              3. Funds disbursed to your bank account after approval.
            </p>
          </div>
...
const BorrowerStatus = ({ borrower, setView }) => {
  ...
  // Load persisted applications from storage on mount (survives refresh)
  useEffect(function() {
    if (!borrower?.id) return;
    var uid = borrower.userId || borrower.id;
    StorageService.getAllAppsForBorrower(uid).then(function(sa) {
      if (sa && sa.length) setStoredApps(sa);
    }).catch(function() {});
  }, [borrower?.id]);

  // Merge: DB.applications + LENDER_DB.applications + storage apps (de-duped)
  var dbApps = DB.applications.filter(function(a) { return a.borrowerId === borrower?.id; });
  var ldbApps = LENDER_DB.applications.filter(function(a) {
    return a.borrowerId === borrower?.id && !dbApps.find(function(d) { return d.id === a.id; });
  });
  var normLdbApps = ldbApps.map(function(a) {
    return {
      id: a.id, borrowerId: a.borrowerId, lenderId: a.lenderId,
      amount: a.amount, term: a.term, purpose: a.purpose,
      status: a.status === "new_lead" ? "pending" : a.status,
      tier: a.tier, createdAt: a.receivedAt ? a.receivedAt.slice(0, 10) : "—",
      riskScore: a.riskScore, matchedLender: null,
    };
  });
  // Include storage apps not already in DB/LENDER_DB
  var allIds = {};
  dbApps.concat(normLdbApps).forEach(function(a) { allIds[a.id] = true; });
  var newStoredApps = storedApps.filter(function(a) { return !allIds[a.id]; });
  var apps = dbApps.concat(normLdbApps).concat(newStoredApps)
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 18 }}>
                  ...
                  {app.amount && app.term && (
                    <div style={{ padding: "10px 16px", borderRadius: 10, background: DS.colors.surfaceAlt, textAlign: "center", flexShrink: 0 }}>
                      <p style={{ fontSize: 10, color: DS.colors.textMuted, textTransform: "uppercase", letterSpacing: "0.05em" }}>Est. Monthly</p>
                      <p style={{ fontWeight: 800, fontFamily: "'DM Mono',monospace", fontSize: 18, color: statusColor }}>
                        N${(app.amount * 1.24 / app.term).toFixed(0)}
                      </p>
                    </div>
                  )}
                </div>
...
Risk Score: ${rr.finalScore}/100 Tier ${rr.tier}. DTI: ${app.dti}. Salary: NAD ${app.salary.toLocaleString()}/mo.
Loan request: NAD ${app.amount.toLocaleString()} over ${app.term} months for ${app.purpose}.
...
const AdminBorrowers = ({ showToast, setView }) => {
  ...
  // Combine DB borrowers + LENDER_DB borrowers for a platform-wide view
  const allBorrowers = [...LENDER_DB.borrowers];
  ...
  const downloadCSV = () => {
    const rows = [["Name","ID","Employer","Salary","Tier","Score","DTI","Status","KYC","AML","Bank","Loans","Date"]];
    allBorrowers.forEach(b => rows.push([b.name,b.idNumber,b.employer,b.salary,b.tier,b.riskScore,b.dti,b.status,b.kycStatus,b.amlStatus,b.bankVerified,b.loans.length,b.assignedDate]));
    const csv = rows.map(r => r.map(v=>`"${v}"`).join(",")).join("\n");
    const blob = new Blob([csv],{type:"text/csv"});
    const url = URL.createObjectURL(blob);
